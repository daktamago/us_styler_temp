...
def process_folders(root_path, param_file_path, output_file):
    ...
    # 원본 컬럼명은 그대로 두고, 'style'이라는 이름의 컬럼만 찾아냅니다.
    style_col = [col for col in df_params.columns if str(col).strip().lower() == 'style'][0]
    df_params[style_col] = df_params[style_col].astype(int)
    df_params = df_params.set_index(style_col)
