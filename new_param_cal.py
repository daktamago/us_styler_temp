...
def process_single_file(file_path, width, height, df_params):
    ...
    try:
        ...
        # style1, style_1, Style 1, STYLE-1 등 다양한 형태를 모두 잡아냅니다.
        style_match = re.search(r'style[_\-\s]*(\d+)', filename, re.IGNORECASE)
        ...
        if style_match:
            ...
            try:
                ...
                # 만약 번호는 찾았는데 엑셀에 그 번호가 없다면 경고 메시지 출력 (원인 파악용)
                print(f"Warning: 파일에서 Style {style_number}을 찾았으나 엑셀에 없습니다. ({filename})")
        else:
            print(f"Warning: 파일명에서 Style 번호를 찾을 수 없습니다. ({filename})")
...
def process_folders(root_path, param_file_path, output_file):
    df_params = pd.read_excel(param_file_path)
    # 컬럼명이 정확히 'style'이 아닐 수 있으므로 소문자로 변환 후 공백 제거
    df_params.columns = df_params.columns.str.strip().str.lower() 
    
    # style 컬럼이름이 다를 경우(예: 'style_no' 등)를 대비해 가장 첫 번째 컬럼을 인덱스로 잡는 방법도 좋습니다.
    # 여기서는 'style' 컬럼을 정수로 강제 변환 후 인덱스로 세팅합니다.
    df_params['style'] = df_params['style'].astype(int)
    df_params = df_params.set_index('style')
