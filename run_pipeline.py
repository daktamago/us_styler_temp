...
def get_user_options():
    ...
    iq_dim = int(get_input("10. IQ Parameter(Input) 개수", "80"))
    ...
    ext_input = input("      (기본값 사용 및 차원 유지 시 엔터 / 예: 512,256): ").strip()
    ...
    return {
        'raw_data_file': raw_data_file, 'train_file': train_file, 'test_file': test_file,
        'scaler_path': scaler_path, 'ref_file': ref_file, 'batch_size': batch_size,
        'epochs': epochs, 'learning_rate': learning_rate, 'step_threshold': step_threshold,
        'iq_dim': iq_dim, 'do_prep': do_prep, 'target_col_file': target_col_file,
        'hidden_dims': hidden_dims, 'extractor_dims': extractor_dims,
        'reg_head_dims': reg_head_dims, 'cls_head_dims': cls_head_dims,
        'train_type': train_type, 'custom_weights': custom_weights,
        'restore': restore, 'eval_filename': eval_filename
    }
...
def main():
    ...
    # 2. 데이터 로드 및 변수 설정
    # ---------------------------------------------
    (X_train_scaled, X_val_scaled, y_train_norm, y_val_norm, scaler_X, 
    ...
    def execute_model_lifecycle(type_name, save_name, iq_idx, style_idx, style_names):
        ...
        y_tr_sub = y_train_norm[:, style_idx]
        y_val_sub = y_val_norm[:, style_idx]
        ...
        model = DynamicSiameseMultiTask(
            ...
            extractor_dims=opt['extractor_dims'], 
        ...
        #  [수정된 부분] 설정 정보를 포함하여 통째로 저장 (Inference 시 자동 로드용)
        checkpoint = {
        ...
        torch.save(checkpoint, f'{save_name}.pth')
