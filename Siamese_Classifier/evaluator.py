...
def evaluate_classifier(model, scaler_X, test_file, ref_file, cls_num_list, iq_dim, style_names, out_file, restore, device):
    ...
    model.eval()
    with torch.no_grad():
        pred_list = model(torch.tensor(X_scaled, dtype=torch.float32).to(device), torch.tensor(X_scaled[tgt_idx], dtype=torch.float32).to(device))
        pred_diff = np.zeros((n_samples, len(style_names)))
        for i, num_cls in enumerate(cls_num_list):
            pred_c = torch.argmax(torch.softmax(pred_list[i], dim=1), dim=1).float()
            pred_diff[:, i] = ((pred_c / (num_cls - 1)) * 2.0 - 1.0).cpu().numpy() if num_cls > 1 else 0.0
