...
def prepare_raw_data(raw_file, ref_file, target_columns, train_file, test_file, test_size=0.1, iq_dim=60):
    ...
    if len(actual_targets) == 0:
        raise ValueError(
            ...
            f"타겟 파일에서 읽어온 이름(일부): {target_cols_cleaned[:5]}"
        )
    ...
    # Reference 파일의 인덱스(Min, Max 등)와 컬럼명을 맵핑하기 쉽게 정제
    ref_df.index = ref_df.index.astype(str).str.strip().upper()
