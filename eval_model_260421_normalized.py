...
# ==========================================
# 9. 전체 데이터 쌍 종합 오차 분석 (MAE + Huber Loss)
# ==========================================
def export_all_pairs_comprehensive(model, scaler_X, scaler_y, test_file_path, output_file_path="test_results_all_comprehensive.xlsx", delta=1.0):
    print(f"\n--- 전체 데이터 종합 테스트 및 엑셀 저장 시작 (MAE & Huber Loss) ---")
    ...
    # 3. 전체 데이터를 한 번씩 모두 Pair로 만들기 (전체 인덱스를 랜덤하게 섞어서 1:1 무작위 매칭)
    # np.random.seed(42)  # 재현성을 원할 경우 주석 해제
    tgt_indices = np.random.permutation(N)
    tgt_data = curr_data[tgt_indices]
    ...
    # (2) 요소별 Huber Loss 계산 (수동 구현)
    # 오차가 delta 이하이면 0.5 * 오차^2 (MSE 방식)
    # 오차가 delta 초과이면 delta * (오차 - 0.5 * delta) (MAE 방식)
    huber_error_raw = np.where(abs_error_raw <= delta, 
                               0.5 * (abs_error_raw ** 2), 
                               delta * (abs_error_raw - 0.5 * delta))
    
    # (3) 각 Pair별 종합 오차 (행 단위 평균)
    pair_mae = np.mean(abs_error_raw, axis=1)
    pair_huber = np.mean(huber_error_raw, axis=1)
    
    # (4) 각 Style 파라미터별 전체 종합 오차 (열 단위 평균)
    overall_style_mae = np.mean(abs_error_raw, axis=0)
    overall_style_huber = np.mean(huber_error_raw, axis=0)
    ...
    # 상단 요약 줄: MAE와 Huber Loss를 각각 1, 2행에 기록
    top_row_mae = ["ALL_PAIRS_SUMMARY", "0_Overall_Style_MAE"] + [None] * iq_dim + overall_style_mae.tolist()
    top_row_huber = ["ALL_PAIRS_SUMMARY", "0_Overall_Style_Huber"] + [None] * iq_dim + overall_style_huber.tolist()
    
    rows.append(top_row_mae)
    rows.append(top_row_huber)
    ...
    for i in range(N):
        ...
        # ★ 추가 사항: 6행에 Huber Loss 기록 (라벨에 해당 Pair의 Huber Loss 명시)
        row6 = [pair_id, f"6_Huber_Loss (Pair_Huber: {pair_huber[i]:.3f})"] + blank_iq + huber_error_raw[i].tolist()
        
        rows.extend([row1, row2, row3, row4, row5, row6])
            
    # DataFrame 변환 및 엑셀 저장
    result_df = pd.DataFrame(rows, columns=columns)
    result_df.to_excel(output_file_path, index=False)
    print(f"✅ 전체 데이터 쌍(총 {N}개) 종합 분석 결과(MAE & Huber 포함)가 '{output_file_path}'에 성공적으로 저장되었습니다.")